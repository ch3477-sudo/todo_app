...
class Todo:
    ...
    def __repr__(self):
        ...
        # repr은 eval()로 다시 객체로 바꿀 수 있는 문자열 형태로 작성하는 게 원칙이다. <- 핵심정리
        # return f'Todo(Task="{self.__task}", done="{self.__done}")'
        # return f'Todo(Task={self.__task!r}, done={self.__done!r})'
...
# Todo 객체를 list에 쌓는 용도의 함수(추가할 할 일을 작성하면 실행되는 함수)
...
st.text_input('새로운 할일 추가', key='new_task', on_change=add_todo) # input 창에 내용을 작성하고 enter하면 add_todo함수 호출 # on_change -> 기존과 하나라도 변화가 있어야 작동
